...
ggplot(avg_data, aes(x = avg_income, y = avg_internet_users)) +
  ...
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Linear Regression: Internet Usage vs. GDP per Capita",
    x = "Average GDP per Capita (USD)",
    y = "Average Internet Usage (%)",
    caption = "Data Source: Gapminder & World Bank"
  ) +
...
model <- lm(avg_internet_users ~ avg_income, data = avg_data)

# Tidy and display coefficients
tidy(model) |>
  kable(digits = 3, caption = "Linear Regression Coefficients: Internet Usage ~ GDP per Capita")
```
